...
class Dancer:
    def __init__(self, host: str, port: int, music_dir: str, logger: Logger, analysis_interval: float = 5):
        ...
        """
        self.music_dir = os.path.join(os.path.dirname(__file__), "musics")
    ...
    def wait_for_server_ready(self, port, timeout=10.0, interval=0.5):
        ...
        self.logger({"msg": f"[INFO] Waiting for Blossom server at {url}..."}, level="info")
        start_time = time.time()
        while time.time() - start_time < timeout:
            try:
                r = requests.get(url)
                if r.status_code == 200:
                    self.logger({"msg": f"[INFO] Blossom server on port {port} is ready."}, level="debug")
                    return True
            except requests.exceptions.RequestException as e:
                self.logger({"msg": f"[ERROR] Blossom server on port {port} is not ready: {e}"}, level="warning")
            time.sleep(interval)
        self.logger({"msg": f"[ERROR] Timeout: Blossom server on port {port} not responding."}, level="error")
        return False
    ...
    def send_sequence(self, sequence_str):
        ...
        try:
            ...
            if response.status_code == 200:
                self.logger({"msg": f"[Dancer] Sequence '{sequence_str}' sent successfully!"}, level="debug")
                return True
            else:
                self.logger({"msg": f"[Dancer] Received status code {response.status_code}"}, level="error")
                return False
        except requests.exceptions.RequestException as e:
            self.logger({"msg": f"[ERROR] Failed to send sequence '{sequence_str}': {e}"}, level="error")
    ...
    def start(self):
        ...
        try:
            ...
            self.logger({"msg": f"[ERROR] Failed to start DANCER server: {e}"}, level="critical")
        finally:
            self.stop()
    ...
    def stop(self):
        ...
        if self.dancer_server_proc:
            self.logger({"msg": "[INFO] Shutting down DANCER server..."}, level="info")
