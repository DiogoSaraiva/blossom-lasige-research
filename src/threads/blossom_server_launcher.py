...
class BlossomServerLauncher(QThread):
    def __init__(self, logger, host, ports: dict, blossom_type: str = "mimetic"):
        super().__init__()
        self.logger = logger
        self.ports = ports
    ...
    def wait_for_server_ready(self, timeout=10.0, interval=0.5):
        url = f"http://{Settings.host}:{self.ports[self.blossom_type]}/"
        self.logger(f"Waiting for Blossom server at {url}...", level="info")
    ...
    def run(self):
        try:
            self.logger(f"Launching blossom_public/start.py ({self.blossom_type.upper()})...", level="info")
